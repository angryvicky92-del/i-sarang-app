...
export const getDaycares = async (arcode = '') => {
  ...
  // 1. Check TTL Cache
  const cached = _cache.get(arcode);
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    return cached.data;
  }

  // 2. Prevent duplicate concurrent requests
  if (_pendingRequests.has(arcode)) {
    return _pendingRequests.get(arcode);
  }
  ...
  const fetchPromise = (async () => {
    ...
    const items = doc.getElementsByTagName("item");
      ...
      _cache.set(arcode, { data: result, timestamp: Date.now() });
      return result;
    } catch (e) {
      console.error('Fetch error 030', e);
      toast.error('오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
      return [];
    } finally {
      _pendingRequests.delete(arcode);
    }
  })();

  _pendingRequests.set(arcode, fetchPromise);
  return fetchPromise;
};
