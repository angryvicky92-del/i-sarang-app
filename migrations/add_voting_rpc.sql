-- Atomic Voting RPC to handle counts without RLS issues
...
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER -- Bypass RLS for counter updates
AS $$
DECLARE
    v_vote_table text;
    v_main_table text;
...
BEGIN
    ...
    -- 4. Return updated row + new user vote status
